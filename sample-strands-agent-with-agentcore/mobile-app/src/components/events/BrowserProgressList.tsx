...
export default function BrowserProgressList({ steps }: Props) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.primaryBg, borderColor: colors.toolChipBorder }]}>
      <View style={styles.header}>
        <Ionicons name="globe-outline" size={14} color={colors.primary} />
        <Text style={[styles.label, { color: colors.primary }]}>Browser steps</Text>
        <View style={[styles.countBadge, { backgroundColor: colors.toolChipBg }]}>
          <Text style={[styles.count, { color: colors.primary }]}>{steps.length}</Text>
        </View>
      </View>
      ...
      <ScrollView style={styles.list} nestedScrollEnabled>
        {hiddenCount > 0 && !showAll && (
          <Pressable onPress={() => setShowAll(true)} style={styles.moreBtn}>
            <Text style={[styles.moreText, { color: colors.primary }]}>+ {hiddenCount} earlier step{hiddenCount !== 1 ? 's' : ''}</Text>
          </Pressable>
        )}
        ...
        {visible.map((step, i) => {
          ...
          return (
            <View key={step.stepNumber} style={styles.row}>
              <View style={[styles.bullet, { backgroundColor: colors.primaryLight }, isLatest && { backgroundColor: colors.primary, width: 8, height: 8 }]} />
              <View style={styles.rowContent}>
                <Text style={[styles.stepNum, { color: colors.primaryLight }, isLatest && { color: colors.primary }]}>
      ...
      {showAll && steps.length > 3 && (
        <Pressable onPress={() => setShowAll(false)} style={styles.moreBtn}>
          <Text style={[styles.moreText, { color: colors.primary }]}>Show fewer</Text>
        </Pressable>
      )}
    </View>
  )
